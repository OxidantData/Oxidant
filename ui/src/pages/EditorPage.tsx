...
export default function EditorPage() {
  ...
  /** Render a terminal status doc into the result/error panes. */
  async function showDoc(d: StatementDoc) {
    setDoc(d);
    if (d.status === "succeeded") {
      setResult(await api.statements.result(d.statementId, 500));
  ...
  async function cancel() {
    ...
    try {
      ...
      /* already terminal — the poll loop surfaces it */
    }
  }

  /** Click a recent statement: reload its status/result into the panes. */
      ...
      <div className="space-y-4">
        <div className="oxidant-card space-y-3">
          ...
          <div className="flex items-center gap-2">
            <button className="oxidant-btn" onClick={run} disabled={running}>
              {running ? "Running…" : "Run"}
            </button>
            <button
        ...
        {doc?.status === "canceled" && (
          <div className="oxidant-card text-muted">Statement canceled.</div>
        )}
        {result && (
          <div className="oxidant-card overflow-x-auto">
            <ResultTable result={result} fullRowCount={doc?.rowCount} />
          </div>
        )}
      </div>
