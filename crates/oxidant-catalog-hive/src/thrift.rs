...
//! three Hive Metastore RPCs Oxidant needs (`get_all_databases`, `get_all_tables`, `get_table`).
//!
...
/// `oxidant-catalog-hive`'s `HiveCatalog::create_table` from the CTAS result's schema (via
/// `oxidant_catalog::hive_types::schema_to_columns`/`format_serde`).
...
/// `parameters` (9, empty), `tableType` (12, `EXTERNAL_TABLE` — Oxidant never owns/deletes the data).
