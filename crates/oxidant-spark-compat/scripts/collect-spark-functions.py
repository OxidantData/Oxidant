#!/usr/bin/env python3
"""Regenerate `crates/oxidant-spark-compat/spark-functions.json` from a reference Spark SQL
language manual's builtin-functions page.

The catalog is the reference half of the `oxidant-parity functions` gap report. It is committed
rather than fetched at runtime so the gap number is reproducible offline and a change to the
documented surface shows up as a reviewable diff.

Usage (from the repo root):

    curl -sSL -A Mozilla/5.0 "$SPARK_FUNCTIONS_MANUAL_URL" -o /tmp/spark-builtin.html
    python3 crates/oxidant-spark-compat/scripts/collect-spark-functions.py /tmp/spark-builtin.html
...
SRC = os.environ.get("SPARK_FUNCTIONS_MANUAL_URL", "Spark SQL builtin-functions reference manual")
h = open(sys.argv[1] if len(sys.argv) > 1 else 'spark-builtin.html').read()
...
def exclusion(key, e):
    ...
    if n.startswith('ai_') or n in VECTOR:
        return ('ai-vector', 'Requires managed model-serving / vector-search infrastructure.')
    if n.startswith('read_') or n in PLATFORM_SOURCE:
        return ('platform-source', 'Requires a managed control plane or an external service.')
    if n in FILES:
        return ('file', 'Requires Unity Catalog volumes.')
    if n in WORKSPACE:
        return ('workspace',
                "Bound to a managed workspace's identity, secrets, metric views, or JVM reflection.")
...
doc = {
    '_comment': ('Spark SQL builtin-function surface, scraped from a reference Spark SQL language '
                 'manual. One entry per distinct function name: overload pages (date_add/date_add3, '
                 ...
                 'entries name why. See docs/spark-functions.md for how to regenerate.'),
    'source': SRC,
    'scraped': '2026-08-29',
    'total': len(funcs),
    'counts': dict(counts),
...
out = os.path.join(os.path.dirname(os.path.abspath(__file__)), os.pardir, 'spark-functions.json')
