...
//! Ready entries are capped by `OXIDANT_STAGE_PLAN_CACHE_ENTRIES` (default
...
impl StagePlanCache {
    ...
    /// operation ([`StagePlanCache::current_cap`]) so `OXIDANT_STAGE_PLAN_CACHE_ENTRIES=0`
    ...
    /// The current entry budget: the `OXIDANT_STAGE_PLAN_CACHE_ENTRIES` override when set
    ...
    fn current_cap(&self) -> usize {
        match std::env::var("OXIDANT_STAGE_PLAN_CACHE_ENTRIES") {
...
/// `OXIDANT_STAGE_PLAN_CACHE_ENTRIES` is re-read on every cache operation
...
/// `shuffle_input__s{S}_p{P}_{i}`) — the names `oxidant_execution::shuffle::localized_
