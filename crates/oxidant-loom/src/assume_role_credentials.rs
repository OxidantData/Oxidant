...
/// `oxidant_catalog::TableMetadata::storage_options`) — deliberately the real Hadoop-AWS key names,
/// not a oxidant-invented equivalent, so a config ported from Spark/Hadoop-AWS just works.
...
impl AssumeRoleCredentialProvider {
    pub fn new(role_arn: String, session_name: Option<String>, region: String) -> Self {
        Self {
            role_arn,
            session_name: session_name.unwrap_or_else(|| "oxidant-cluster".to_string()),
            region,
...
mod tests {
    ...
    fn fresh_cached_is_none_before_first_refresh() {
        let p = AssumeRoleCredentialProvider::new(
            "arn:aws:iam::123456789012:role/oxidant-poolctl/analytics".to_string(),
            None,
            "us-west-2".to_string(),
        );
    ...
    fn default_session_name_is_stable() {
        ...
        assert_eq!(p.session_name, "oxidant-cluster");
    }
}
