...
//! is about to read:
//!
//! - Delta/Iceberg: the pinned [`oxidant_datasource::SnapshotIdentity`] the driver resolved
...
//!
//! Total decoded bytes (deep Arrow size) are capped by `OXIDANT_DIM_CACHE_BYTES` (default 2 GiB;
...
impl DimCache {
    ...
    /// Whether this cache stores anything (`OXIDANT_DIM_CACHE_BYTES=0` disables caching).
...
/// The process-global cache, sized once from `OXIDANT_DIM_CACHE_BYTES` (default
...
/// Parse the `OXIDANT_DIM_CACHE_BYTES` budget (absent/unparseable → default).
fn cap_bytes_from_env() -> u64 {
    parse_cap_bytes(std::env::var("OXIDANT_DIM_CACHE_BYTES").ok().as_deref())
}
...
pub fn fingerprint_snapshot(snapshot: &oxidant_datasource::SnapshotIdentity) -> String {
...
mod tests {
    ...
    fn fingerprint_snapshot_distinguishes_versions_and_formats() {
        let delta7 = oxidant_datasource::SnapshotIdentity::Delta { version: 7 };
        let delta8 = oxidant_datasource::SnapshotIdentity::Delta { version: 8 };
        ...
        let iceberg = oxidant_datasource::SnapshotIdentity::Iceberg {
            snapshot_id: 7,
            sequence_number: 0,
