...
//! is process-global env (`WEFT_WORKER_COUNT` / `WEFT_SHARD_INDEX`): process isolation keeps
...
use weft_catalog::{CatalogProvider as WeftCatalog, Result as CatResult, TableMetadata};
...
impl WeftCatalog for FakeCatalog {
    ...
    async fn load_table(&self, ns: &[String], table: &str) -> CatResult<TableMetadata> {
        if ns == ["ns"] && table == "orders" {
            ...
            Err(weft_catalog::Error::Plan(format!("no such table {table}")))
        }
    }
}
...
async fn count_sum_under_overlay(engine: &Engine, overlay: &str) -> (i64, i64) {
    let batches = weft_loom::shard::with_replicated_tables(overlay, async {
        engine
