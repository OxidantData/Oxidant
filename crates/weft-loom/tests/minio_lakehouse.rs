...
fn minio_enabled() -> bool {
    std::env::var("WEFT_MINIO_TEST")
        .ok()
        .is_some_and(|value| value == "1" || value.eq_ignore_ascii_case("true"))
}

fn endpoint() -> String {
    std::env::var("WEFT_MINIO_ENDPOINT").unwrap_or_else(|_| "http://127.0.0.1:9000".into())
}
...
async fn minio_parquet_iceberg_and_delta_are_correct() {
    if !minio_enabled() {
        eprintln!("skipping MinIO lakehouse integration test; set WEFT_MINIO_TEST=1");
        return;
    }
