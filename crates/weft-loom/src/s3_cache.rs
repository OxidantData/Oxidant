...
//! local NVMe: the first read of an object materializes it under `WEFT_S3_CACHE_DIR`, later
...
//! revalidate against S3 `HEAD` (size + etag) after `WEFT_S3_CACHE_TTL_MS` so an overwritten
...
impl DiskCachingStore {
    ...
    /// Build the wrapper from `WEFT_S3_CACHE_DIR` (unset/empty disables caching),
    /// `WEFT_S3_CACHE_MAX_BYTES` (default 20 GiB) and `WEFT_S3_CACHE_TTL_MS` (default 300000).
    pub fn from_env(inner: Arc<dyn ObjectStore>) -> Arc<dyn ObjectStore> {
        let Some(dir) = std::env::var("WEFT_S3_CACHE_DIR")
            .ok()
            .filter(|s| !s.trim().is_empty())
        else {
            return inner;
        };
        let max_bytes = std::env::var("WEFT_S3_CACHE_MAX_BYTES")
            .ok()
            .and_then(|s| s.trim().parse().ok())
            .unwrap_or(DEFAULT_MAX_BYTES);
        let ttl_ms = std::env::var("WEFT_S3_CACHE_TTL_MS")
            .ok()
            .and_then(|s| s.trim().parse().ok())
        ...
        match Self::new(
            ...
            Ok(store) => {
                eprintln!(
                    "weft: S3 disk cache enabled at {dir} (max {max_bytes} B, ttl {ttl_ms} ms)"
                );
                Arc::new(store)
            }
            Err(e) => {
                eprintln!("warn: WEFT_S3_CACHE_DIR={dir} unusable ({e}); reading S3 direct");
                inner
            }
        }
    }
