...
impl LakeFormationAuth {
    ...
    /// resolves as option → `AWS_REGION` → `AWS_DEFAULT_REGION` → shared profile / IMDS.
...
/// Resolve the AWS region: option → `AWS_REGION` → `AWS_DEFAULT_REGION` → shared profile / IMDS. Env
