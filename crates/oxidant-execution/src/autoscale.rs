...
//!
//! When `OXIDANT_AUTOSCALE=1` and `OXIDANT_GATEWAY_URL` + `OXIDANT_CLUSTER_ID` are set, the driver
...
pub fn autoscale_enabled() -> bool {
    std::env::var("OXIDANT_AUTOSCALE")
        .ok()
        .is_some_and(|v| v == "1" || v.eq_ignore_ascii_case("true"))
}

/// Task slots per worker (`OXIDANT_WORKER_TASK_SLOTS`, default = available parallelism).
...
/// Gateway URL and cluster id for proactive scale-up (`OXIDANT_GATEWAY_URL`, `OXIDANT_CLUSTER_ID`).
pub fn autoscale_target() -> Option<(String, String)> {
    let url = std::env::var("OXIDANT_GATEWAY_URL")
        .ok()
        .filter(|s| !s.is_empty())?;
    let id = std::env::var("OXIDANT_CLUSTER_ID")
        .ok()
        .filter(|s| !s.is_empty())?;
...
mod tests {
    ...
    /// Serialize env-mutating tests — `OXIDANT_AUTOSCALE` / gateway vars are process-global.
    ...
    fn autoscale_enabled_accepts_one_and_true() {
        let _guard = env_lock();
        std::env::remove_var("OXIDANT_AUTOSCALE");
        assert!(!autoscale_enabled());
        std::env::set_var("OXIDANT_AUTOSCALE", "1");
        assert!(autoscale_enabled());
        std::env::set_var("OXIDANT_AUTOSCALE", "TRUE");
        assert!(autoscale_enabled());
        std::env::set_var("OXIDANT_AUTOSCALE", "yes");
        assert!(!autoscale_enabled());
        std::env::remove_var("OXIDANT_AUTOSCALE");
    }
    ...
    fn autoscale_target_requires_both_url_and_cluster_id() {
        let _guard = env_lock();
        std::env::remove_var("OXIDANT_GATEWAY_URL");
        std::env::remove_var("OXIDANT_CLUSTER_ID");
        assert!(autoscale_target().is_none());

        std::env::set_var("OXIDANT_GATEWAY_URL", "http://gateway:8080/");
        assert!(autoscale_target().is_none());

        std::env::set_var("OXIDANT_CLUSTER_ID", "demo");
