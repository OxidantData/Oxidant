...
//!
//! Behind the `k8s` feature flag. Resolves `OXIDANT_WORKER_SERVICE` (e.g.
//! `oxidant-worker.oxidant-cl-abc.svc.cluster.local`) to live pod IPs at query scheduling time.
...
impl K8sMembership {
    /// Build from environment: `OXIDANT_WORKER_SERVICE` and optional `OXIDANT_WORKER_PORT` (default 50561).
    pub fn from_env() -> Option<Self> {
        let host = std::env::var("OXIDANT_WORKER_SERVICE").ok()?;
        if host.is_empty() {
            return None;
        }
        let port = std::env::var("OXIDANT_WORKER_PORT")
            .ok()
