//! Worker honours `OXIDANT_MEMORY_LIMIT_BYTES` for shuffle caches (SpillStore) without OOM.
...
async fn worker_memory_limit_spills_shuffle_buckets() {
    ...
    // Build engines without the memory cap; workers pick up OXIDANT_MEMORY_LIMIT_BYTES at serve time
    ...
    tokio::spawn(async move {
        std::env::set_var("OXIDANT_MEMORY_LIMIT_BYTES", "1");
