//! Dependency-aware concurrent stage dispatch (`OXIDANT_CONCURRENT_STAGES`, default off;
//! the tests opt in via `OXIDANT_CONCURRENT_STAGES=1`):
...
//!    race.
//! 2. `sequential_dispatch_when_disabled` — `OXIDANT_CONCURRENT_STAGES=0` restores the legacy
...
/// `OXIDANT_CONCURRENT_STAGES` is process-global; serialize every test in this binary for
...
async fn independent_arms_overlap_and_match_single_node() {
    ...
    std::env::set_var("OXIDANT_CONCURRENT_STAGES", "1");
...
async fn sequential_dispatch_when_disabled() {
    let _guard = ENV_LOCK.lock().unwrap();
    std::env::set_var("OXIDANT_CONCURRENT_STAGES", "0");
    let planner = full_engine();
    let expected = rows_sorted(&planner.sql(Q58_SHAPE).await.unwrap());
    let dq = plan(&planner, Q58_SHAPE).await;
    let arms = arm_stage_ids(&dq);
    ...
    let events = collect_events(&store);
    let actual = run_distributed_observed(&cluster, &dq, &store).await;
    std::env::remove_var("OXIDANT_CONCURRENT_STAGES");
    assert_eq!(
        rows_sorted(&actual),
        expected,
    ...
    assert_eq!(
        first_wave.len(),
        1,
        "OXIDANT_CONCURRENT_STAGES=0 must dispatch one stage at a time; events={events:?}"
    );
}
    ...
    let failed_arm_finished = events.iter().any(|e| {
        matches!(
            ...
            ExecutionEvent::TaskFinished {
                stage_id: 1,
                status: oxidant_observability::TaskStatus::Failed,
                ..
            }
        )
    });
