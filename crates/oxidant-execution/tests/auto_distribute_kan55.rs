...
async fn q16_self_exists_global_count_distinct_plans_gathered_distinct() {
    ...
    assert!(
        dq.stages
            .iter()
            .any(|s| s.sql.contains("__oxidant_semi_gate")),
...
async fn q9_scalar_projection_merges_same_tail_scalars_into_one_scan() {
    ...
    assert!(outer.sql.contains("__oxidant_scalar_src"), "{}", outer.sql);
    assert!(
...
async fn anti_only_replicated_outer_keeps_the_gather() {
    ...
    assert!(
        dq.stages
            .iter()
            .any(|s| s.sql.contains("__oxidant_subquery_gate")),
...
async fn subquery_only_tables_are_size_classified() {
    ...
    let lp = engine
        ...
        .await
        .unwrap();
    let replicated = oxidant_execution::plan::resolve_replicated_tables(&engine, &lp).await;
    assert!(
