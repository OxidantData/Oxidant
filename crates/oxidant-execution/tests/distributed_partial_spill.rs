...
use oxidant_execution::flight::serve_worker_with_spill;
use oxidant_execution::shuffle::spill::{estimated_batch_bytes, SpillStore};
...
async fn skewed_workers_partial_spill_matches_no_spill() {
    ...
    tokio::spawn({
        let e0 = e0.clone();
        async move {
            let _ = oxidant_execution::flight::serve_worker(p0, e0).await;
        }
    });
    tokio::spawn({
        let e1 = e1.clone();
        async move {
            let _ = oxidant_execution::flight::serve_worker(p1, e1).await;
        }
    });
