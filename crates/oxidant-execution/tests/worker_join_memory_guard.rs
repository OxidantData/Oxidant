...
//! and returns correct results. KAN-45: the fallback is opt-in (`OXIDANT_SORT_MERGE_FALLBACK`)
...
/// `OXIDANT_SORT_MERGE_FALLBACK` opt-in, no `OXIDANT_PREFER_HASH_JOIN` force. The engine must
