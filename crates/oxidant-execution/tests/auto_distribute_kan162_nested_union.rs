...
//! - **(b) ROLLUP variant** (q5-authentic): producers gather to partition 0,
...
//!   table; a nested union sitting under a non-inner join; and q5's real web leg (the
//!   nested union's returns branch is `web_returns LEFT JOIN web_sales`, scanning the
//!   sharded fact a second time with the replicated side preserved) — that leg needs a
//!   co-located key shuffle, a separate composition, and must keep declining here.
...
async fn assert_matches_single_node(tag: &str, sql: &str, check_plan: impl Fn(&DistributedQuery)) {
    ...
    let dq = plan_strict(&lp, &REPL, tag);
    check_plan(&dq);
...
/// would repeat unmatched web_returns rows on every worker. That shape needs a co-located
/// key shuffle of web_sales (a separate composition); until then it must keep declining.
...
#[tokio::test]
async fn q5_web_leg_left_join_over_sharded_fact_declines_safely() {
    assert_declines("q5-web-leg", Q5_WEB_LEG).await;
}
