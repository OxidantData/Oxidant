...
//! worker with the flipped join strategy (`oxidant_loom::with_join_strategy_flipped`) before
...
async fn watchdog_abort_retries_stage_once_with_flipped_join_strategy() {
    ...
    std::env::set_var("OXIDANT_TEST_STAGE_STALL_ONCE_MS", "30000");
    ...
    std::env::remove_var("OXIDANT_TEST_STAGE_STALL_ONCE_MS");
}
