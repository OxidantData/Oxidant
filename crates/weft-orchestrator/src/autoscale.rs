...
use weft_execution::autoscale::{ParallelismDemand, ScaleRecommendation};
...
pub fn recommend_for_cluster(
    ...
    weft_execution::autoscale::recommend_worker_count(
        current,
        bounds.min_workers,
        bounds.max_workers,
        demand,
...
) -> weft_common::Result<Option<ClusterInfo>> {
    if !rec.should_scale {
        return Ok(None);
    }
...
mod tests {
    ...
    use weft_execution::autoscale::parallelism_demand;
    use weft_execution::driver::{Cluster, StageDef};
