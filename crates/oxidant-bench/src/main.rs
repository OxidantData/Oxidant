//! `oxidant-bench` — benchmark harness for ClickBench coverage.
//!
...
//!   [`oxidant_loom::Engine`] (DataFusion) over an in-memory table. Fast; the CI coverage gate.
//! - `clickbench-grpc` — live-server: writes synthetic `hits.parquet`, boots the real
//!   `oxidant-connect` Spark Connect server, and drives `CREATE EXTERNAL TABLE` + the 43 queries
...
//!
//! Usage: `cargo run -p oxidant-bench [--release] -- {clickbench|clickbench-grpc} [--rows N]`
...
/// Live-server: boot oxidant-connect, register `hits` (synthetic parquet, or a real
...
async fn run_clickbench_grpc(rows: usize, data: Option<String>) {
    ...
    let (label, machine, data_size) = if let Some(path) = data.as_deref() {
        ...
        let dir = std::env::temp_dir().join("oxidant-bench");
...
async fn run_correctness_distributed(_rows: usize, data: Option<String>) {
    ...
    let tpcds_sample: usize = std::env::var("OXIDANT_DIST_TPCDS_SAMPLE")
        .ok()
...
// See oxidant-cli: generated stage SQL can recurse past tokio's 2 MiB default thread stack inside
