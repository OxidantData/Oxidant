...
pub async fn run(sf: f64, dir: &Path) {
    ...
    let allow_no_oracle = std::env::var("OXIDANT_TPCDS_ALLOW_NO_ORACLE").is_ok();
    let oracle = tpcds_data::duckdb_path();
    match &oracle {
        Some(p) => eprintln!("[tpcds] oracle: DuckDB at {p}\n"),
        None if allow_no_oracle => {
            eprintln!(
                "[tpcds] WARNING: DuckDB not found — OXIDANT_TPCDS_ALLOW_NO_ORACLE set; execute-only (no result check)\n"
            );
        }
        None => {
            eprintln!(
                "[tpcds] DuckDB not found on PATH — required for oracle cross-check \
                 (set OXIDANT_TPCDS_ALLOW_NO_ORACLE=1 to allow execute-only smoke)"
            );
            std::process::exit(1);
        }
    }
            ...
            Some(p) => match duckdb_result(p, dir, sql) {
                ...
                Some(expected) => {
                    ...
                    if rows_equal(&got, &want) {
                        ...
                        if std::env::var("OXIDANT_TPCDS_DEBUG").is_ok() {
                            eprintln!(
                                "  mismatch: oxidant {} rows, duckdb {} rows",
                                got.len(),
                                want.len()
                            );
                            for w in got.iter().take(3) {
                                eprintln!("  sample-oxidant:   {w:?}");
                            }
    ...
    // Any query failure fails the process (including OXIDANT_TPCDS_ONLY single-query debug).
    if failed > 0 {
