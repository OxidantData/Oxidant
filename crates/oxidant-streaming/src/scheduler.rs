...
impl StreamingQueryManager {
    ...
    pub async fn run_batch(&self, query_id: &str, engine: &Engine) -> oxidant_common::Result<u64> {
        let mut guard = self.queries.write().await;
        let m = guard
            .get_mut(query_id)
            .ok_or_else(|| oxidant_common::Error::Execution("unknown query".into()))?;
    ...
    match config.sink_format.to_ascii_lowercase().as_str() {
        "parquet" | "json" | "csv" => {
            let path = config
                ...
                .unwrap_or_else(|| "/tmp/oxidant-stream-out".into());
