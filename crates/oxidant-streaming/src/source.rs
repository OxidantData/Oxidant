...
impl FileSource {
    pub fn new(path: impl AsRef<Path>, format: &str) -> Self {
        Self {
            ...
            seen: HashSet::new(),
            table_name: "_oxidant_stream_src".into(),
        }
    }
...
/// Kafka micro-batch source. Reads JSON lines from a spool directory (`OXIDANT_KAFKA_SPOOL`) or
...
impl KafkaSource {
    pub fn from_options(options: &std::collections::HashMap<String, String>) -> Self {
        let topic = options
            ...
            .cloned()
            .unwrap_or_else(|| "oxidant".into());
        let brokers = options
            .get("kafka.bootstrap.servers")
            .or_else(|| options.get("bootstrap.servers"))
            .cloned()
            .unwrap_or_else(|| "localhost:9092".into());
        let spool = std::env::var("OXIDANT_KAFKA_SPOOL")
            .unwrap_or_else(|_| format!("/tmp/oxidant-kafka-{topic}"));
        Self {
            topic,
            brokers,
