...
//! Run `cargo build -p oxidant-cli` before `cargo test -p oxidant-cli --test cli_fault_tolerance`.
...
impl WorkerHandle {
    fn spawn(oxidant: &Path, port: u16, data: &Path, fault_env: &[(&str, &str)]) -> Self {
    ...
    fn respawn(&mut self, oxidant: &Path) {
        ...
        cmd.env_remove("OXIDANT_FAULT_EXIT_ON_TASK");
        cmd.env_remove("OXIDANT_FAULT_EXIT_STAGE");
...
impl DistQuerySetup {
    async fn new(n: i64, fault_w0: &[(&str, &str)]) -> Self {
        ...
        let p0 = pick_port();
        let p1 = pick_port();
        let w0 = WorkerHandle::spawn(&oxidant, p0, &p0_path, fault_w0);
        let w1 = WorkerHandle::spawn(&oxidant, p1, &p1_path, &[]);
...
async fn run_with_worker_restart(fault_w0: &[(&str, &str)]) -> bool {
    ...
    let oxidant = setup.oxidant.clone();
    ...
    let driver = tokio::task::spawn_blocking(move || {
        for _ in 0..120 {
            let out = Command::new(&oxidant)
                ...
                .env("OXIDANT_TASK_MAX_RETRIES", "12")
                .output()
                .expect("run driver");
    ...
    for _ in 0..240 {
        if setup.w0.try_wait().is_some() {
            setup.w0.respawn(&setup.oxidant);
...
async fn cli_producer_worker_kill_restart_matches_single_node() {
    let ok = run_with_worker_restart(&[
        ("OXIDANT_FAULT_EXIT_ON_TASK", "1"),
        ("OXIDANT_FAULT_EXIT_STAGE", "producer"),
    ])
    .await;
    assert!(
        ok,
...
async fn cli_consumer_worker_kill_recompute_matches_single_node() {
    let ok = run_with_worker_restart(&[
        ("OXIDANT_FAULT_EXIT_ON_TASK", "1"),
        ("OXIDANT_FAULT_EXIT_STAGE", "consumer"),
    ])
    .await;
    assert!(
        ok,
