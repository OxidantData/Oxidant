...
async fn observability_records_local_sql_job() {
    ...
    let svc = OxidantService::with_store(store.clone());
    ...
    let op = "test-op-1";
    let mut tracker =
        oxidant_observability::QueryTracker::begin(store.clone(), op, "SELECT x FROM t");
    ...
    assert_eq!(
        store.operation_state(op),
        Some(oxidant_observability::OperationState::Succeeded)
    );
