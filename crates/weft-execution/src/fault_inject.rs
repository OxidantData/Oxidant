//! Test-only worker fault injection (env: `WEFT_FAULT_EXIT_ON_TASK`, `WEFT_FAULT_EXIT_STAGE`).
//!
...
fn stage_matches_filter(ticket: &StageTicket) -> bool {
    match std::env::var("WEFT_FAULT_EXIT_STAGE") {
...
pub fn maybe_fault_exit(ticket: &StageTicket) {
    let Ok(raw) = std::env::var("WEFT_FAULT_EXIT_ON_TASK") else {
        return;
    };
    ...
    if n == exit_on {
        eprintln!(
            "weft worker fault injection: exit on matching task {n} \
             (stage_id={}, produce={})",
