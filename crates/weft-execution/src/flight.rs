...
pub fn stage_timeout() -> std::time::Duration {
    let ms = std::env::var("WEFT_STAGE_TIMEOUT_MS")
        .ok()
        .and_then(|s| s.parse().ok())
        .filter(|&ms: &u64| ms > 0)
        .unwrap_or(600_000);
...
/// Retention for cached stage outputs (env: `WEFT_STAGE_OUTPUT_TTL_SECS`, default 1 hour;
...
/// forever. Entries are swept lazily on insert (same idiom as weft-connect's `CompletedOps`).
pub fn stage_output_ttl() -> std::time::Duration {
    std::env::var("WEFT_STAGE_OUTPUT_TTL_SECS")
        .ok()
        .and_then(|s| s.parse().ok())
...
/// Server-side per-stage no-progress budget (env: `WEFT_STAGE_NO_PROGRESS_SECS`, default 10
...
pub fn stage_no_progress_budget() -> std::time::Duration {
    let secs = std::env::var("WEFT_STAGE_NO_PROGRESS_SECS")
        .ok()
        .and_then(|s| s.parse().ok())
        .filter(|&s: &u64| s > 0)
        .unwrap_or(600);
...
struct ProgressSample {
    ...
    /// DataFusion + weft shuffle spill bytes on disk (worker-wide signal, frozen under the
...
fn stage_summary_line(
    ...
    let mut line = format!(
        "Weft stage summary: stage_id={stage_id} partition_id={partition_id} \
...
/// plus the weft shuffle spill dir (KAN-47 progress signal + observability).
fn total_spill_bytes(engine: &Engine, spill: Option<&SpillStore>) -> u64 {
    engine.spill_dir_bytes() + spill.map_or(0, |s| weft_loom::dir_bytes(s.root()))
}
    ...
    tokio::select! {
        res = tokio::time::timeout(timeout, run) => match res {
            ...
            Err(_) => Err(Status::resource_exhausted(format!(
                "stage timed out after {} ms (WEFT_STAGE_TIMEOUT_MS)",
                timeout.as_millis()
            ))),
        },
...
/// Test hook (KAN-17 tests): `WEFT_TEST_STAGE_DELAY_MS` sleeps inside the bounded stage body so
/// the timeout and cancel paths can hold a task slot deterministically. Never set in production.
async fn test_stage_delay() {
    let Some(ms) = std::env::var("WEFT_TEST_STAGE_DELAY_MS")
        .ok()
...
/// Test hook (KAN-47 tests): `WEFT_TEST_STAGE_BATCH_DELAY_MS` sleeps after each batch in
/// the stage streaming paths so a slow-but-progressing stage can be simulated. Never set
/// in production.
async fn test_batch_delay() {
    let Some(ms) = std::env::var("WEFT_TEST_STAGE_BATCH_DELAY_MS")
        .ok()
        .and_then(|s| s.parse::<u64>().ok())
        .filter(|&ms| ms > 0)
    else {
...
/// Test hook (KAN-53 tests): `WEFT_TEST_STAGE_STALL_ONCE_MS` sleeps inside the bounded
...
async fn test_stage_stall_once() {
    ...
    let Some(ms) = std::env::var("WEFT_TEST_STAGE_STALL_ONCE_MS")
        .ok()
        .and_then(|s| s.parse::<u64>().ok())
        .filter(|&ms| ms > 0)
    else {
    ...
    fn stage_watchdog(
        ...
        let sample = move || ProgressSample {
            ...
            spill_bytes: engine.spill_dir_bytes()
                + spill_root.as_deref().map_or(0, weft_loom::dir_bytes),
        };
                ...
                let result = match result {
                    Err(status) if is_no_progress_abort(&status) => {
                        ...
                        let retry = async {
                            test_stage_delay().await;
                            test_stage_stall_once().await;
                            weft_loom::with_join_strategy_flipped(
...
mod tests {
    ...
    fn align_batch_schema_rejects_genuine_mismatch() {
        ...
        let foreign = RecordBatch::try_new(
            ...
            vec![Arc::new(weft_loom::arrow::array::StringArray::from(vec![
                "x",
            ]))],
        )
        .unwrap();
    ...
    async fn do_exchange_streams_large_partition_under_memory_budget() {
        ...
        // files exist under the default weft-shuffle-spill prefix. do_exchange pushes land in
    ...
    fn no_progress_abort_detection_is_specific() {
        ...
        assert!(!is_no_progress_abort(&Status::resource_exhausted(
            "stage timed out after 600000 ms (WEFT_STAGE_TIMEOUT_MS)"
        )));
