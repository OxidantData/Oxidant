...
use weft_loom::arrow::array::{Array, RecordBatch, StringViewArray, UInt32Array};
...
fn compact_views(batch: RecordBatch) -> Result<RecordBatch> {
    ...
    let cols = batch
        .columns()
        .iter()
        .map(|col| -> Result<weft_loom::arrow::array::ArrayRef> {
...
mod tests {
    ...
    fn string_view_buckets_do_not_retain_source_buffers() {
        use weft_loom::arrow::array::StringViewArray;
