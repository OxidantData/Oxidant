...
//! cluster (`WEFT_WORKER_COUNT` > 1 on the driver) the stage now runs on EVERY worker, each
...
// ENV_LOCK serializes process-global `WEFT_WORKER_COUNT` / `WEFT_REPLICATED_TABLES` across
// async tests.
...
use weft_execution::driver::{run_stages, ExchangeMode};
use weft_execution::plan::{plan_distributed, plan_distributed_logical, DistributedQuery};
use weft_loom::arrow::array::{ArrayRef, Int64Array};
use weft_loom::arrow::datatypes::{DataType, Field, Schema};
use weft_loom::arrow::record_batch::RecordBatch;
use weft_loom::shard::ShardAssignment;
use weft_loom::Engine;

/// `WEFT_WORKER_COUNT` / `WEFT_REPLICATED_TABLES` are process-global; serialize the tests
/// that mutate them.
...
impl weft_catalog::CatalogProvider for SliceCatalog {
    fn name(&self) -> &str {
        "testcat"
    }
    ...
    async fn load_table(
        ...
        Ok(weft_catalog::TableMetadata::new(
            table,
            location,
...
fn set_w2_env() {
    ...
    std::env::remove_var("WEFT_POD_NAME");
}
...
fn replicated_stage(dq: &DistributedQuery) -> &weft_execution::driver::StageDef {
    dq.stages
        .iter()
...
/// An operator force-include (`WEFT_REPLICATED_TABLES`) wins over slicing: the workers' env
...
async fn env_forced_replicated_tables_keep_forward() {
    let _guard = ENV_LOCK.lock().unwrap();
    set_w2_env();
    std::env::set_var("WEFT_REPLICATED_TABLES", "store_sales,web_sales");
    let dir = tempfile::tempdir().unwrap();
    let engine = catalog_engine(fixtures(dir.path())).await;
    let lp = engine.logical_plan(Q71_SHAPE).await.unwrap();
    let dq = plan_distributed_logical(&lp, &REPLICATED).expect("Q71 shape must plan");
...
fn rows_sorted(batches: &[RecordBatch]) -> Vec<Vec<String>> {
    let opts = weft_loom::arrow::util::display::FormatOptions::default().with_null("NULL");
    let mut rows = Vec::new();
    for b in batches {
        let fmts: Vec<_> = b
            .columns()
            .iter()
            .map(|c| weft_loom::arrow::util::display::ArrayFormatter::try_new(c, &opts).unwrap())
            .collect();
...
async fn two_workers(root: &std::path::Path) -> weft_execution::driver::Cluster {
    ...
    for (i, port) in [p0, p1].into_iter().enumerate() {
        ...
        tokio::spawn(async move {
            let _ = weft_execution::flight::serve_worker_with_assignment(
                port,
                e,
    ...
    weft_execution::driver::Cluster::new(vec![
