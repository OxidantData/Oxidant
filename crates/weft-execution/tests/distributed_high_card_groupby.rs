...
//! single-node exactly, with `WEFT_SHUFFLE_PARTITIONS` *greater* than the worker count.
//!
...
use weft_execution::flight::{serve_worker, serve_worker_with_spill};
use weft_execution::shuffle::spill::SpillStore;
...
/// The Q18 stage shape over `orders`/`lineitem` (see `WEFT_TPCH_DEBUG=1` output of
/// `weft-bench tpch-distributed`): leaf partial-agg → HAVING semi-filter → leaf join →
...
async fn wait_for_workers(endpoints: &[String]) {
    for ep in endpoints {
        for _ in 0..50 {
            if weft_execution::flight::health_check_worker(ep.clone())
                .await
                .is_ok()
            {
                break;
...
async fn high_cardinality_groupby_q18_shape_at_scale() {
    ...
    let spill_root = std::env::temp_dir().join(format!("weft-kan32-spill-{}", std::process::id()));
