...
/// `WEFT_WORKER_TASK_SLOTS` is process-global; serialize the tests that mutate it.
...
async fn concurrent_partition_tasks_on_one_worker_match_single_node() {
    ...
    std::env::set_var("WEFT_WORKER_TASK_SLOTS", "8");
    const N: i64 = 20_000;
    ...
    std::env::remove_var("WEFT_WORKER_TASK_SLOTS");
}
...
async fn tasks_beyond_slot_count_queue_instead_of_rejecting() {
    let _env = ENV_LOCK.lock().await;
    std::env::set_var("WEFT_WORKER_TASK_SLOTS", "1");
    const N: i64 = 8_000;
    let plan = group_by_plan();
