//! AQE shuffle-partition coalescing applied end-to-end (`WEFT_AQE=1`): a producer stage
...
use weft_execution::driver::{run_stages_obs, Cluster, DistributedPlan};
use weft_execution::flight::serve_worker;
use weft_execution::shuffle::hash_partition;
...
/// `WEFT_AQE` / `WEFT_SHUFFLE_PARTITIONS` are process-global; serialize these tests.
...
async fn coalesced_groupby_matches_single_node(workers: i64, np: u32) {
    ...
    std::env::set_var("WEFT_SHUFFLE_PARTITIONS", np.to_string());
    ...
    assert_eq!(
        ...
        "WEFT_SHUFFLE_PARTITIONS must pin np"
    );
