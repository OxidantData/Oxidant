...
/// Escape hatch (`WEFT_REPLICATE_MAX_ROW_MULTIPLE=0`): byte-only legacy classification —
...
async fn two_workers_q37() -> weft_execution::driver::Cluster {
...
async fn two_workers_q72() -> weft_execution::driver::Cluster {
    let (p0, p1) = (unique_worker_port(), unique_worker_port());
    for (i, port) in [p0, p1].into_iter().enumerate() {
        let e = Arc::new(Engine::new());
