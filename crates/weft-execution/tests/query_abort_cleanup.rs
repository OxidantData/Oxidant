...
/// `WEFT_TEST_STAGE_DELAY_MS` / `WEFT_STAGE_TIMEOUT_MS` are process-global; serialize tests.
...
fn spill_root(tag: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("weft-kan46-{tag}-{}", std::process::id()));
...
async fn dropped_driver_future_cancels_stage_and_clears_cache() {
    ...
    std::env::set_var("WEFT_TEST_STAGE_DELAY_MS", "3000");
