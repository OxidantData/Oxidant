//! Adaptive join-order re-optimization end-to-end (`WEFT_REOPT_JOIN_ORDER=1`): the driver
...
//! join in the chain, or with `WEFT_STAGE_INPUT_STATS=0` (no leaf measurements), the
//! driver must run the original plan untouched: correct results, no event.

use std::sync::Arc;

use weft_execution::driver::{reopt_join_order_enabled, run_stages_obs, Cluster, ReoptContext};
...
/// `WEFT_REOPT_JOIN_ORDER` / `WEFT_STAGE_INPUT_STATS` are process-global; serialize.
...
fn batch(cols: Vec<(&str, Vec<i64>)>) -> RecordBatch {
    ...
    RecordBatch::try_new(
        schema,
        cols.into_iter()
            .map(|(_, v)| Arc::new(Int64Array::from(v)) as Arc<dyn weft_loom::arrow::array::Array>)
            .collect(),
    )
    .unwrap()
}
...
async fn reopt_reorders_tail_and_matches_single_node() {
    ...
    std::env::set_var("WEFT_REOPT_JOIN_ORDER", "1");
    ...
    std::env::remove_var("WEFT_REOPT_JOIN_ORDER");
...
/// `WEFT_STAGE_INPUT_STATS=0`: no barrier measurements, so the trigger finds no complete
/// leaf sample and skips the re-optimization — results still correct.
#[tokio::test]
async fn reopt_skips_cleanly_without_stage_input_stats() {
    let _guard = ENV_LOCK.lock().await;
    std::env::set_var("WEFT_REOPT_JOIN_ORDER", "1");
    std::env::set_var("WEFT_STAGE_INPUT_STATS", "0");
