...
//!   `WEFT_DISTRIBUTED_STRICT=1` (no whole-fact gather substitution).
...
async fn strict_plan(planner: &Engine, sql: &str) -> weft_execution::plan::DistributedQuery {
...
async fn q6_stage_sql_has_no_cross_join_anywhere() {
    ...
    assert!(
        ...
        "the four equijoins plan as hash joins under WEFT_PREFER_HASH_JOIN=true:\n{display}"
    );
}
