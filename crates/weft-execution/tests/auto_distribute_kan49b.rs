...
//! table replicated, `WEFT_DISTRIBUTED_STRICT=1` so the whole-fact gather can never substitute).
//!
//! Shapes under test:
//!
...
fn assert_no_fact_gather(dq: &weft_execution::plan::DistributedQuery) {
    assert!(
        !dq.stages
            .iter()
