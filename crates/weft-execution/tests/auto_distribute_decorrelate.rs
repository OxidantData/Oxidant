...
/// Q11 with the `__WEFT_SF__` placeholder (KAN-30) resolved — SF=1 for the general tests.
fn q11(sf: &str) -> String {
    Q11.replace("__WEFT_SF__", sf)
}
...
async fn q11_uncorrelated_threshold_plans_one_row_broadcast() {
    ...
    assert!(
        outer_combine
            .sql
            .contains("WHERE ((r0 > '__WEFT_SCALAR_STAGE__'))"),
...
async fn uncorrelated_threshold_with_min_and_subquery_on_left_matches_single_node() {
    ...
    assert!(
        dq.stages[3]
            .sql
            .contains("WHERE (('__WEFT_SCALAR_STAGE__' < r0))"),
...
// turns this Error::Unsupported into the query error; see weft-connect distributed.rs) ---
