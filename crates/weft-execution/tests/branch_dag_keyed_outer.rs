...
use weft_loom::arrow::array::{ArrayRef, Int64Array, RecordBatch, StringArray};
...
async fn two_workers_q4() -> weft_execution::driver::Cluster {
    ...
    for (i, port) in [p0, p1].into_iter().enumerate() {
        ...
        tokio::spawn(async move {
            let _ = weft_execution::flight::serve_worker(port, e).await;
        });
    }
...
async fn two_workers_q39() -> weft_execution::driver::Cluster {
    let (p0, p1) = (unique_worker_port(), unique_worker_port());
    for (i, port) in [p0, p1].into_iter().enumerate() {
        let e = Arc::new(Engine::new());
...
async fn two_workers_q78() -> weft_execution::driver::Cluster {
    let (p0, p1) = (unique_worker_port(), unique_worker_port());
    for (i, port) in [p0, p1].into_iter().enumerate() {
        let e = Arc::new(Engine::new());
        e.register_batches("store_sales", shard_rows(&store_sales(), i))
...
async fn assert_distributed_matches_single_node(
    sql: &str,
    replicated: &[&str],
    cluster: weft_execution::driver::Cluster,
    ...
    let mut out = None;
    for _ in 0..150 {
        match weft_execution::driver::run_stages(&cluster, &dq.stages).await {
            Ok(b) => {
                out = Some(b);
                break;
            }
