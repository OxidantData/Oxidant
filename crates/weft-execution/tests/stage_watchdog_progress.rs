...
use weft_execution::flight::{health_check_worker, run_stage_on_worker, serve_worker};
...
async fn slow_but_progressing_stage_is_not_aborted() {
    ...
    std::env::set_var("WEFT_STAGE_TIMEOUT_MS", "30000");
    std::env::set_var("WEFT_STAGE_NO_PROGRESS_SECS", "1");
    std::env::set_var("WEFT_BATCH_SIZE", "1");
    std::env::set_var("WEFT_TEST_STAGE_BATCH_DELAY_MS", "150");
    ...
    std::env::remove_var("WEFT_TEST_STAGE_BATCH_DELAY_MS");
}
