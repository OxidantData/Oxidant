...
//!
//! - **Floor**: in strict mode (`WEFT_DISTRIBUTED_STRICT=1`) the whole-fact gather is a fast
...
fn unique_worker_port() -> u16 {
    ...
    if prev == 0 {
        let seed = 46000 + (std::process::id() as u16 % 20000);
...
/// `WEFT_DISTRIBUTED_STRICT` is process-global; serialize the tests that touch the gather path.
...
async fn q15_derived_scalar_plans_one_row_broadcast() {
    ...
    assert!(
        outer
            .sql
            .contains("revenue.total_revenue = '__WEFT_SCALAR_STAGE__'"),
