...
//! once per task (`weft_loom::stage_plan_cache`). A stage's tasks share one cached logical
...
//! - `WEFT_STAGE_PLAN_CACHE_ENTRIES=0` restores the pre-cache path (plan every task).
//!
//! The cache is process-global; each test's workers are fresh engines, and the engine id is
//! a key component, so tests never see each other's entries. `WEFT_STAGE_PLAN_CACHE_ENTRIES`
...
use weft_loom::stage_plan_cache::{global as plan_cache, StagePlanCacheStats};
use weft_loom::Engine;

/// `WEFT_STAGE_PLAN_CACHE_ENTRIES` is process-global; serialize these tests.
...
/// `WEFT_STAGE_PLAN_CACHE_ENTRIES=0` disables the cache: every task plans fresh (no builds,
/// no hits) and results are unchanged.
#[tokio::test]
async fn disabled_cache_plans_every_task() {
    let _guard = ENV_LOCK.lock().await;
    std::env::set_var("WEFT_STAGE_PLAN_CACHE_ENTRIES", "0");
    const CUSTS: i64 = 200;
    const ORDERS: i64 = 2_000;
    let expected = rows(&single_node(ORDERS, CUSTS).await);
    let (cluster, _, _) = two_worker_cluster(ORDERS, CUSTS).await;
