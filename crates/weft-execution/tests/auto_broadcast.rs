//! Auto-broadcast dims from Parquet sizes (KAN-1 / E-DIST-BCAST): no `WEFT_REPLICATED_TABLES`.
...
use weft_execution::plan::{plan_distributed, resolve_replicated_tables};
...
async fn auto_broadcast_parquet_distributed_matches_single_node() {
    ...
    assert_eq!(
        ...
        "auto-broadcast (no WEFT_REPLICATED_TABLES) must match single-node"
    );
}
