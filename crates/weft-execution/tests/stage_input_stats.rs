...
//! action, default on via `WEFT_STAGE_INPUT_STATS`) and ships the per-bucket totals on the
...
//!   safety valve stays);
//! - `WEFT_STAGE_INPUT_STATS=0` restores the pre-A3 path: no measured counts on the
...
/// `WEFT_STAGE_INPUT_STATS` / `WEFT_TARGET_PARTITIONS` / `WEFT_BATCH_SIZE` are
...
/// (d): `WEFT_STAGE_INPUT_STATS=0` restores the pre-A3 path — the driver ships no measured
...
async fn stage_input_stats_disabled_restores_plain_registration() {
    ...
    assert_eq!(
        ...
        0,
        "WEFT_STAGE_INPUT_STATS=0 must bypass measured-stats registration"
    );
    assert_eq!(
        e0.plan_time_smj_reroute_count() + e1.plan_time_smj_reroute_count(),
        0,
