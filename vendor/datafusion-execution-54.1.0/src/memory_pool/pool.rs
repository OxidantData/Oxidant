...
impl MemoryPool for FairSpillPool {
    ...
    fn try_grow(&self, reservation: &MemoryReservation, additional: usize) -> Result<()> {
        ...
        // Oxidant: bound aggregate fallible growth as well as each fair share.
        // Late consumers and split reservations can leave existing allocations
        // above the current share. Subtract separately to avoid overflowing the
        // tracked total, including after infallible grow has exceeded capacity.
        let remaining = self
            .pool_size
            .checked_sub(state.unspillable)
            .and_then(|remaining| remaining.checked_sub(state.spillable));
        if remaining.is_none_or(|remaining| additional > remaining) {
