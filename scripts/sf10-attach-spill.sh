#!/usr/bin/env bash
# Attach a dedicated spill volume to every running oxidant-sf10 instance.
...
# (KAN-57). This gives each node a 200GB gp3 at /var/lib/oxidant/spill,
...
aws ec2 describe-instances --region "$REGION" \
  --filters "Name=tag:aws:autoscaling:groupName,Values=oxidant-sf10-driver,oxidant-sf10-workers" "Name=instance-state-name,Values=running" \
  ...
  if [ "$EXISTING" != "None" ] && [ -n "$EXISTING" ]; then
    ...
    VOL=$(aws ec2 create-volume --region "$REGION" --availability-zone "$AZ" \
          ...
          --tag-specifications "ResourceType=volume,Tags=[{Key=Name,Value=oxidant-sf10-spill},{Key=cluster,Value=oxidant-sf10}]" \
  ...
  if [ "$ASG" = "oxidant-sf10-driver" ]; then
  ...
  ssh -n "${SSH_OPTS[@]}" ${EXTRA[@]+"${EXTRA[@]}"} "$TARGET" '
    ...
    sudo mkdir -p /var/lib/oxidant/spill
    mountpoint -q /var/lib/oxidant/spill || sudo mount $DEV /var/lib/oxidant/spill
    sudo chown oxidant:oxidant /var/lib/oxidant/spill
    echo "oxidant-sf10-spill $DEV on /var/lib/oxidant/spill" | sudo tee /etc/oxidant/spill-device >/dev/null
    df -h /var/lib/oxidant/spill | tail -1' && echo "  mounted at /var/lib/oxidant/spill" || echo "  MOUNT FAILED on $PRIV ($ASG)"
done
echo "== spill volumes attached on all nodes; restart oxidant services to use them =="