...
def qualify_sql(sql: str, glue_db: str, tables: list[str]) -> str:
    """Rewrite bare TPC table refs to glue.<db>.<table> (longest names first)."""
    out = sql
    for t in sorted(tables, key=len, reverse=True):
        out = re.sub(rf"(?i)\b{re.escape(t)}\b", f"glue.{glue_db}.{t}", out)
    # Avoid double-qualifying if a query already used glue.db.table
    out = re.sub(rf"(?i)\bglue\.glue\.{re.escape(glue_db)}\.", f"glue.{glue_db}.", out)
    return out
