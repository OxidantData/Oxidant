...
#
# Tag→env stamping contract (KAN-139, enforced in render_env / worker_count_or_fail):
# an empty or missing instance tag NEVER stamps its OXIDANT_* var (the engine default
# applies), a non-numeric value is ignored with a loud boot warning, and an explicit
# OXIDANT_SHUFFLE_PARTITIONS below the engine floor max(200, worker_vcpus) warns loudly
# at boot — empty CFN params must not pin ShufflePartitions or WorkerCount.
# Shell-level regression tests: deploy/packer/tests/test_env_stamping.sh.
#
...
# Render the oxidant.env contents for ${role} on stdout (pure — no fs side effects, so
# deploy/packer/tests can exercise it directly). Warnings go to stderr via log(); only
# validated KEY=VALUE lines reach stdout. KAN-139 stamping contract:
#   - an EMPTY or missing tag NEVER stamps its env var (the engine default then applies);
#   - a non-numeric value is ignored with a loud warning (never stamped as garbage);
#   - a numeric OXIDANT_SHUFFLE_PARTITIONS below the engine floor max(200, worker_vcpus)
#     IS stamped (explicit operator intent wins) but logs a loud boot warning — a CFN
#     ShufflePartitions=32 silently sliced an SF100 TPC-DS run into 32 buckets.
render_env() {
  local role="$1"
  cat <<EOF
...
EOF
    ...
    if [[ -n "${SHUFFLE_PARTITIONS}" && "${SHUFFLE_PARTITIONS}" != "None" ]]; then
      if [[ "${SHUFFLE_PARTITIONS}" =~ ^[0-9]+$ ]] && (( SHUFFLE_PARTITIONS > 0 )); then
        if (( SHUFFLE_PARTITIONS < 200 )); then
          log "WARNING: oxidant:shuffle-partitions=${SHUFFLE_PARTITIONS} is below the engine" \
            "default floor max(200, worker_vcpus) — a tiny fan-out caused SF100 TPC-DS" \
            "skew/OOM (KAN-139). Leave the CFN ShufflePartitions param empty (or set >=200)" \
            "unless you are deliberately overriding."
        fi
        echo "OXIDANT_SHUFFLE_PARTITIONS=${SHUFFLE_PARTITIONS}"
      else
        log "WARNING: ignoring non-numeric oxidant:shuffle-partitions='${SHUFFLE_PARTITIONS}'" \
          "— engine default max(200, worker_vcpus) applies"
      fi
    fi
...
# KAN-139: clustered roles must never guess the cluster size from an empty/garbage
# oxidant:worker-count tag — the empty-CFN failure class (a silently wrong
# OXIDANT_WORKER_COUNT stamps wrong ASG waits, shard modulus, and env) fails closed
# here instead. Standalone keeps the 1 default (membership vars are unused there).
...
write_env() {
  ...
  # systemd EnvironmentFile: quote values that contain ';' or spaces (render_env handles).
  render_env "${role}" > "${tmp}"
...
oxidant_bootstrap_main() {
  ...
  WORKER_COUNT="$(worker_count_or_fail "${ROLE}" "${WORKER_COUNT}")" || return 1
