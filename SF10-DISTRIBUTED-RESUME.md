# SF10 Distributed Weft — Resume / Handoff
...
- Goal: prove Weft **distributed** execution runs **TPC-H + TPC-DS** end-to-end (Spark Connect → driver → 2 Flight workers), cheaply.
...
## 2. Live infrastructure (weft-sf10)

CloudFormation stack **`weft-sf10`** (region **us-west-2**), status UPDATE_COMPLETE.
...
- Driver: `WEFT_DISTRIBUTED_STRICT=1`, `WEFT_PREFER_HASH_JOIN=false`, `WEFT_SHUFFLE_PARTITIONS=16`, `WEFT_MEMORY_LIMIT_BYTES=8589934592` (8 GiB pool), `WEFT_WORKER_COUNT=2`, `WEFT_WORKER_SERVICE=workers.weft-sf10.weft.internal`; systemd cgroup `MemoryMax=15G`/`MemoryHigh=13G`.
- Workers: `WEFT_MEMORY_LIMIT_BYTES=17179869184` (16 GiB pool), `WEFT_SHUFFLE_SPILL_BYTES=4294967296` (4 GiB), `WEFT_SHARD_INDEX` 0/1; cgroup `MemoryMax=28G`/`MemoryHigh=24G`.

**SSH access**
- Key: `~/.ssh/id_ed25519_kaicoder03` (AWS keypair `weft-debug`), user `ec2-user`.
...
Source: DuckDB prebuilt `tpch-sf10.db` / `tpcds-sf10.db` downloaded to `/tmp/weft-sf10/`.

**Key decision:** export fact tables as **MULTIPLE Parquet files** (not one big file). Weft shards by *whole file* (size-weighted LPT in `crates/weft-loom/src/shard.rs`), so a single-file table lands entirely on ONE worker — that was the SF100 skew root cause. Multi-file is what lets both workers share scans.

- Export script: **`/tmp/weft-sf10/export_sf10.py`** (venv `/tmp/weft-datagen-venv`). Splits facts by `key % N`:
...
- S3: `s3://weft-artifacts-810738286322/tpch-sf10/<table>/` and `.../tpcds-sf10/<table>/`.
...
2. **`deploy/cloudformation/weft-cluster.yaml`** — added **`ShufflePartitions`** parameter (default falls back to `WorkerCount`) and wired it to the `weft:shuffle-partitions` tag on driver+worker launch templates (was hardcoded to `${WorkerCount}`). Bootstrap reads the tag → `WEFT_SHUFFLE_PARTITIONS`.
...
5. **`bench/sf100/run-spark-connect.py`** — added **`--query-timeout`** (default 300s, env `WEFT_QUERY_TIMEOUT`): wraps `df.collect()` in a `ThreadPoolExecutor` with a timeout; on timeout it records a fail and **recreates the Spark session** (so a wedged query fails fast instead of hanging the suite).
6. **Docs** (`docs/distributed-ec2.md`, `bench/sf100/README.md`, `deploy/helm/weft/values-sf100.yaml`) — updated SF100 topology notes: m8g.4xlarge under-provisioned for SF100 joins (HashJoin not spillable); documented memory invariant + `WEFT_SHUFFLE_PARTITIONS`.

**Driver AMI caveat:** `ami-062afcea2d6401e07` ships a **buggy** `/usr/local/lib/weft/bootstrap.sh` (deadlock in `enable_role_unit` + wrong spill device). The repo's `deploy/packer/files/bootstrap.sh` is the **fixed** version. Every fresh instance must be patched: copy the repo script over `/usr/local/lib/weft/bootstrap.sh`, `systemctl reset-failed weft-bootstrap`, re-run it, then start the role unit. **Proper follow-up: rebake the AMI** (`deploy/packer/build-ami.sh`) so this isn't manual.
...
/tmp/weft-smoke-venv/bin/python3 bench/sf100/run-spark-connect.py \
...
- **Whole-file sharding**: SF100 tables were each **one** Parquet file → one worker did all scanning, the other idled. Fixed for SF10 by multi-file export. Spark/Trino split by **row-group/byte-range**; Weft should too (see §9).
...
- **Coarse shuffle** (`WEFT_SHUFFLE_PARTITIONS=2`) caused skew.
...
4. **Engine improvement (bigger):** row-group/byte-range sharding in `crates/weft-loom/src/shard.rs` (split large Parquet files across workers) — the "like Spark" fix from §7.
...
```bash
aws cloudformation delete-stack --stack-name weft-sf10 --region us-west-2
# confirm: aws ec2 describe-instances --region us-west-2 \
#   --filters 'Name=tag:aws:cloudformation:stack-name,Values=weft-sf10' \
...
- Local venvs: `/tmp/weft-smoke-venv` (pyspark client), `/tmp/weft-datagen-venv` (duckdb/pyiceberg/deltalake/s3fs/boto3).
- Data workdir: `/tmp/weft-sf10/` (DBs + export script).
...
- Engine planner: `crates/weft-execution/src/plan/{stage_planner,shape_extensions,join_chain}.rs`; sharding `crates/weft-loom/src/shard.rs`; memory pool `crates/weft-loom/src/lib.rs` (`WEFT_MEMORY_LIMIT_BYTES` → `FairSpillPool`).
