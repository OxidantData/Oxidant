...
| `oxidant spark server --foreground` | The same server in the foreground, for a supervisor that owns the process (systemd, Docker, CI). **Bare `oxidant spark server` is refused** — long-lived processes are daemons |
