...
them are checked we can say — without asterisks — that Oxidant distributes the load.
...
| Area | Reality today |
|------|---------------|
| Entry point | `RelType::Sql` only — [crates/oxidant-connect/src/lib.rs](../crates/oxidant-connect/src/lib.rs) `base_relation_batches`. DataFrame relations lower via `translate` to the local engine. |
...
| Non-aggregate queries | **Partial (D-2.2).** `try_non_aggregate` in [shape_extensions.rs](../crates/oxidant-execution/src/plan/shape_extensions.rs) lowers scan/filter/sort/limit over one sharded table (scatter + global finalize) and all-replicated scans (Forward). `peel()` also strips `SubqueryAlias` / WHERE filters. CTE-heavy cross joins (Q1/Q4/…) still reject — aggregate lives inside the CTE. |
| Fallback | Silent. [distributed.rs](../crates/oxidant-connect/src/distributed.rs) maps `Error::Unsupported` to `Ok(None)` and discards the reason. |
| Fault tolerance | **Implemented, unproven.** [scheduler.rs](../crates/oxidant-execution/src/scheduler.rs) has retries, alternate-worker failover, health checks, lineage recompute, speculation (`OXIDANT_SPECULATIVE`, default off). No failure-injection test. |
| Exchange | `do_exchange` in [flight.rs](../crates/oxidant-execution/src/flight.rs) buffers the whole stream into a `Vec` before caching — push transport, not streaming. |
| Sharding | Round-robin `i % N` over a sorted file list ([shard.rs](../crates/oxidant-loom/src/shard.rs)) — disjoint but size-blind, so skewed files skew workers. |
...
- [x] **D-0.2 Strict mode.** `OXIDANT_DISTRIBUTED_STRICT=1` turns fallback into an error
...
([crates/oxidant-bench/src/tpcds.rs](../crates/oxidant-bench/src/tpcds.rs), `bench/tpcds/queries/`).
Turn "many shapes fall back" into an exact number before any cloud spend.

- [x] **D-1.1 Planner-coverage harness.** `oxidant-bench tpcds-distributed` runs
...
- [x] **D-2.1 DataFrame API routing.** Route translated Connect relations through the
  ...
  *(`plan_distributed_logical` + `try_run_distributed_plan`;
  `crates/oxidant-connect/tests/distributed_pyspark.rs`.)*
...
- [x] **D-2.3 Ranking and `ORDER BY` windows.** `ROW_NUMBER`/`RANK`/`DENSE_RANK`/`LAG`/`LEAD`
  ...
  [shape_extensions.rs](../crates/oxidant-execution/src/plan/shape_extensions.rs) — ranking is
...
- [x] **D-2.5 `UNION` distinct / `INTERSECT` / `EXCEPT`** — branch stages plus a
  ...
  that lives under `E-DIST-BCAST` / `OXIDANT_AUTO_BROADCAST_THRESHOLD_BYTES`.)*
...
- [x] **D-3.1 Failure injection.** Kill a worker mid-stage and assert a correct result via
  ...
  *(`cargo test -p oxidant-cli --test cli_fault_tolerance` + `OXIDANT_FAULT_EXIT_*`.)*
- [x] **D-3.2 Speculation default.** With D-3.1 in place, measure straggler benefit and
  decide whether `OXIDANT_SPECULATIVE` defaults on; record the decision.
...
- [x] **D-3.4 Spill parity on the streaming path.** The ticket/cache path spills
  (`OXIDANT_SHUFFLE_SPILL_BYTES`); the streaming path must too.
...
- [x] **D-3.6 Autoscaling on query parallelism** in
  [crates/oxidant-orchestrator/src/backend.rs](../crates/oxidant-orchestrator/src/backend.rs),
  replacing idle-pod scaling.
- [x] **D-3.7 Worker memory budget.** Workers honour a per-pod limit and degrade by
  spilling rather than being OOM-killed, mirroring the driver's `OXIDANT_MEMORY_LIMIT_BYTES`.
...
- [~] **D-4.3** `DISTRIBUTED_SF100=1 WORKER_MIN=2 WORKER_MAX=2 ./bench/sf100/run-time-gate.sh`
  with `OXIDANT_DISTRIBUTED_STRICT=1`, so any fallback fails the run rather than producing a
  driver-only number labelled distributed.
  *(Gate script exports `OXIDANT_DISTRIBUTED_STRICT=1`. Preferred honesty path is EC2 CF
...
| Gate | Command |
|------|---------|
| Planner coverage ratchet | `cargo run -p oxidant-bench -- tpcds-distributed` (D-1.1/1.3) |
| Execute correctness ratchet | `cargo run -p oxidant-bench -- tpcds-distributed --execute --sf 0.01 --workers 2` (D-1.2) |
| Distributed correctness | `cargo run -p oxidant-bench -- correctness-distributed` |
| TPC-H distributed | `cargo run -p oxidant-bench -- tpch-distributed --sf 0.01 --workers 2` |
| Fault tolerance | worker-kill test from D-3.1 in `cargo test --workspace` |
| Strict mode | SF100 gate run with `OXIDANT_DISTRIBUTED_STRICT=1` (D-4.3) |

## Explicit non-goals for "done"

Out of scope for this claim, so it cannot creep: adaptive re-optimization beyond the
[aqe.rs](../crates/oxidant-execution/src/aqe.rs) stub, cost-based shuffle partition sizing,
