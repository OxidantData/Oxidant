...
| `OXIDANT_S3_CACHE_MAX_OBJECT_BYTES` | Optional | Per-object materialization cap (default `2147483648` = 2 GiB). Larger objects skip the cache and stay on ranged S3 GETs (`0` disables the cap). Required for single-file SF100 facts (~21 GiB `lineitem.parquet`). |
...
| `OXIDANT_PREFER_HASH_JOIN` | Optional | `auto` (default) \| `true` \| `false` (KAN-53). `auto` chooses the join strategy **per query**, matching Spark `preferSortMergeJoin=true`: large equi-joins take spillable sort-merge. With a bounded pool the fallback order is: (1) build-side estimate over `min(pool × OXIDANT_HASH_JOIN_MAX_BUILD_FRACTION, spark_shj_cap)` ⇒ sort-merge re-plan; (2) **no usable estimate ⇒ sort-merge**; (3) under budget ⇒ hash join; (4) runtime pool exhaustion ⇒ one sort-merge retry. `spark_shj_cap` is Spark's `canBuildLocalHashMap` rule (`OXIDANT_HASH_JOIN_PER_PARTITION_THRESHOLD_BYTES × shuffle_partitions / 2`). Without a bounded pool there is no budget, so plans keep their hash joins. `true`/`false` force one strategy session-wide; legacy `1`/`0`/`on`/`off` spellings are accepted. |
