...
| `OXIDANT_SAMPLE_DATA` | Optional | `1`/`true`/`yes`/`on` enables the bundled sample-data tree without `--sample-data` (systemd-friendly). `--sample-data` and `OXIDANT_SAMPLE_DATA_DIR` still win. When enabled and no tree is discovered next to the binary, the path is `sample-data`. |
