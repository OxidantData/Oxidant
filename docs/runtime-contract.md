...
| `OXIDANT_BROADCAST_JOIN_THRESHOLD_BYTES` | Optional | Spark `autoBroadcastJoinThreshold` analogue for KAN-142 runtime broadcast conversion (default `10485760` = 10 MiB; `0` disables). Under `auto` with a bounded pool, a partitioned INNER hash join whose build side is positively measured (stage-barrier row counts) or estimated at or below this threshold — clamped to the build budget — converts to a broadcast (`CollectLeft`) hash join, eliding both sides' shuffle repartitions within the stage. |
