...
| E-LOOM-FLAKE-2 | Tests | Open | Same FairSpillPool family, different sub-mechanism, seen once under load: `auto_runtime_retry_when_estimate_underreports` — the SMJ retry's `ExternalSorterMerge` reservation was denied while the doomed hash attempt's reservations hadn't drained (`wait_for_pool_drain`'s 10s cap expires under CPU starvation). Product-level exposure behind it: `Engine::sort_merge_physical_plan` can produce a many-sorter plan that hard-fails on a tight bounded pool at high core counts (the 1 MiB `sort_spill_reservation_bytes` lowering doesn't bound the first *batch* reservation) — consider capping `target_partitions` for bounded-pool SMJ re-plans; needs bench validation | `crates/weft-loom/src/lib.rs` |
