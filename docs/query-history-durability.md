...
$OXIDANT_DATA_DIR/                  # default ./oxidant-data next to checkpoints
  ...
  logs/
    ...
    oxidant-2026-08-23.log          # daily roll (default)
    oxidant-2026-08-23-14.log       # when OXIDANT_LOG_ROLL=hourly
    oxidant-2026-W34.log            # when OXIDANT_LOG_ROLL=weekly
```
...
- Roll on the configured clock boundary: `daily` default, `hourly`/`weekly` via
  `OXIDANT_LOG_ROLL`. Naming as in §3. At roll time and at boot, rolled files
  older than `OXIDANT_LOG_KEEP_DAYS` (default **30**) are unlinked.
- `GET /api/v1/logs` gains `?file=current|YYYY-MM-DD[‑HH]` so the Observability
  screen pages back the full 30 days; absent file → 404, same honesty as the
  connector-log endpoint.
