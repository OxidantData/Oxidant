...
| Math functions | Partial | **Stale — was 13/20.** `e`, `bit_get`, `negative`, `width_bucket`, `hypot`, `bit_reverse`, `rint`, `expm1`, `log1p`, `sec`, `csc`, `mod`, `pmod` and `ceiling` are now registered (`spark_functions/spark_math2.rs`, plus the alias tables). Still missing: `randn`, `uniform`. Diverging: `bround(2.5)` → `bround on non-integral input (Float64) is not supported`. Working: `abs`, `ceil`, `floor`, `round`, `exp`/`ln`/`log`/`log2`/`log10`, `pow`/`power`/`sqrt`/`cbrt`, all trig and hyperbolic, `degrees`/`radians`/`pi`, `sign`/`signum`, `greatest`/`least`, `factorial`, `conv`, `shiftleft`/`shiftright`/`shiftrightunsigned`, `bit_count`, `try_add`/`try_subtract`/`try_multiply`/`try_divide`, `isnan`, `nanvl`. | KAN-92, KAN-93 |
