# Oxidant — Resume Guide & Next Steps
...
**Oxidant is a working drop-in Spark replacement that beats Sail's published ClickBench on real
...
cargo run -p oxidant-bench -- clickbench       # 43/43 engine-direct
cargo run -p oxidant-bench -- clickbench-grpc   # 43/43 through the live server over gRPC
cargo run -p oxidant-bench -- correctness       # gRPC == engine-direct + ground-truth anchors
cargo run -p oxidant-bench -- tpch              # TPC-H Q1/Q3/Q5/Q6/Q10
# run the server, then point any PySpark client at sc://localhost:50051:
cargo run --bin oxidant -- spark server --port 50051
```
...
- `c6a.sh` runs `bench/clickbench/benchmark.sh` semantics: `oxidant-bench clickbench-grpc --data <hits.parquet>`
  with `OXIDANT_MEMORY_LIMIT_BYTES=26000000000` (spill pool) and `OXIDANT_TARGET_PARTITIONS`.
...
Knobs are env-tunable in `oxidant-loom::Engine::new` (no rebuild to sweep): `OXIDANT_TARGET_PARTITIONS`,
`OXIDANT_BATCH_SIZE`, `OXIDANT_COALESCE_BATCHES`, `OXIDANT_REPARTITION_AGGREGATIONS` (`schema_force_view_types`
...
| `OXIDANT_TARGET_PARTITIONS=4` | 0.872 s | +137% (Q32 8× worse) |
| `OXIDANT_TARGET_PARTITIONS=8` | 0.435 s | +18% |
| `OXIDANT_BATCH_SIZE=32768` | 0.345 s | −6% (Q23/Q28) |
| `OXIDANT_REPARTITION_AGGREGATIONS=off` | 0.674 s | +83% (Q32 7× worse) |
...
Implemented in `crates/oxidant-execution`: multi-stage `partial-agg → hash shuffle → final-agg`,
...
- `driver::run_distributed` + `oxidant worker` / `oxidant driver` CLI subcommands; task-slot scheduling
  and per-task retry/fallback are in the driver path.
- `oxidant spark server --mode local-cluster --workers N` starts in-process Flight workers for local
  Spark Connect smoke tests.
- `oxidant-connect::distributed` prefers the distributed path whenever workers are configured; if the
...
broadcast joins, global ORDER BY/LIMIT, shuffle spill (`OXIDANT_SHUFFLE_SPILL_DIR`), dynamic worker
...
- **PySpark parity — DONE, validated against stock PySpark** (`crates/oxidant-connect`):
  `pyspark-connect 4.0` on Python 3.11 (`scratchpad/venv`) runs
  `spark.sql(...).{collect,toPandas,show}()`, DDL create+collect, GROUP BY/AVG, filters, `range()`
  end-to-end. Implemented: `SqlCommand.input` (query → lazy `SqlCommandResult`; DDL → eager exec +
...
- **DataFrame API — DONE, validated against stock PySpark** (`oxidant-connect::translate`): Spark
...
- **gRPC must allow 128 MB+** and **chunk** result batches (`oxidant-connect`: `MAX_MSG`/`CHUNK_ROWS`);
  the default 4 MB limit fails real results.
- **`OXIDANT_MEMORY_LIMIT_BYTES`** sets a DataFusion spill pool — required so heavy queries spill
  instead of OOM-killing on a 32 GB box.
- **tonic versions split on purpose:** `oxidant-connect`/`oxidant-proto` use **0.12** (Spark Connect);
  `oxidant-execution` uses **0.14** (matches `arrow-flight 58`). They don't exchange tonic types.
...
- **Shuffle proto alignment is fine:** `datafusion-proto 54`, `arrow-flight 58`, and `prost 0.14`
  all resolve to **prost 0.14.4** in `oxidant-execution` (verified) — the shuffle envelope and the
...
- **Lakehouse:** the `deltalake`/`iceberg` crates pin DataFusion 53 → can't compose with our 54.
  We instead resolve tables to their active Parquet file list (`oxidant_datasource::delta_active_files`
...
## Map of the code
`crates/`: `oxidant-connect` (Spark Connect gRPC server; `translate::{expr,relation}` lowers the
DataFrame API to DataFusion, `types` does Arrow↔Spark) · `oxidant-proto` (generated protos via protox) ·
`oxidant-loom` (DataFusion 54 engine + lakehouse register) · `oxidant-datasource` (Delta/Iceberg file
resolvers) · `oxidant-execution` (Flight driver/worker + `shuffle::{protocol,partition,codec}` +
`driver::run_distributed`) · `oxidant-bench` (ClickBench/TPC-H/correctness
harness) · `oxidant-{plan,analyzer,optimizer,physical,catalog,hvm,common,cli}` (mostly scaffold).
