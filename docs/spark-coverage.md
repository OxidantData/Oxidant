# Spark SQL coverage matrix

Section-by-section map of the Spark SQL language manual onto what the Oxidant engine
actually does today, across four axes: **statements**, **functions**, **data types**, and
**operators**.

This is the KAN-89 deliverable for the "Oxidant Spark SQL compatibility on AWS Glue + Lake
Formation" epic. Every **Missing** or **Partial** row below names the ticket that owns closing it
(original epic stories KAN-89..KAN-108, plus follow-on tickets KAN-110..KAN-118 filed from this
matrix).
...
| Probes run | 578 statements over three passes (508 first pass = 505 scored across the four axes below + 3 setup/DDL warm-ups not attributed to an axis, 55 follow-ups, 15 semantic checks). Reconstructible statement probes are listed in [`parity/spark-probes.md`](../parity/spark-probes.md); function-category ratios are category aggregates (see that file). |
| Date | 2026-08-09 |
...
| **Partial** | Some of the section works; a named part does not, or it parses but the semantics diverge from Spark. |
| **Missing** | Probe failed outright (parser or planner rejects it) and no code path implements it. |
| **N/A** | Requires a managed lakehouse platform's control plane (Unity Catalog *securables and DDL*, Delta Sharing, serverless, workspace identities, AI/Model Serving). Out of scope for this matrix. Unity Catalog as a read-only *metadata source* is a separate, partially working thing — see [catalogs-unity.md](catalogs-unity.md). |
...
> **The original planning document's figures are superseded.** Its "Current state" section
> reported strict 536 / semantic 4,984 of 12,641. Those numbers are stale; the file it cited
> (`parity/baseline.json`) now reads 2,854 / 7,890 of 14,706 as tabulated above. Read the baseline
> file for current parity numbers.

The Spark corpus measures Spark SQL parity, not platform-specific surface. It is the right
denominator for the function/dialect tickets and the wrong one for statements outside core Spark
SQL (`COPY INTO`, Delta maintenance, Unity Catalog DDL), which is why the matrix below exists.
...
not assert value-identical results against a reference Spark implementation, so these are upper
bounds). Denominators sum to 505; the
...
| `PUT INTO` | N/A | Not probed. Managed-platform volume-ingest statement, control-plane only. | — |
...
| `TABLESAMPLE` | Partial | Parses but is ignored: `SELECT count(*) FROM cov_t TABLESAMPLE (1 ROWS)` → `3` on a 3-row table (the reference implementation returns 1). `TABLESAMPLE (50 PERCENT)` likewise returns everything. | KAN-111 |
...
| Named parameter markers (`:param`) | N/A | Client-side binding done by the SQL client driver, not engine SQL surface. | — |
...
| `DESCRIBE TABLE EXTENDED` | Partial | `DESCRIBE TABLE EXTENDED cov_t` → OK but returns exactly the same 3 column rows as plain `DESCRIBE`; the reference implementation appends the `# Detailed Table Information` block. | KAN-98 |
...
| `DESCRIBE FUNCTION` | Partial | `DESCRIBE FUNCTION upper` → `Function: upper`, but `Class: N/A` and `Usage: N/A` where the reference implementation returns the real class and usage string. | KAN-98 |
...
Managed lakehouse platforms typically express privileges through Unity Catalog. The in-scope
analogue for Oxidant is AWS Lake Formation, which the epic tackles as a plan-rewrite/authorization
layer (KAN-102..KAN-104) rather than as SQL `GRANT` statements.
...
> 244-function subset at engine commit `b3f3f2e`. [`spark-functions.md`](spark-functions.md)
> now measures the same axis **exactly and reproducibly** — it diffs oxidant's *live* function
> registry against all 606 documented Spark function names, and is regenerated by
...
> `crates/oxidant-spark-compat/spark-functions.json`.
...
| `CHAR(n)` / `VARCHAR(n)` | Partial | Both parse and cast to `Utf8View`, but the length is not applied: `SELECT concat('[', CAST('ab' AS CHAR(4)), ']')` → `[ab]`, where the reference implementation pads to `[ab  ]`. | KAN-117 |
...
| `MAP<K,V>` | Partial | Values are right — `SELECT map('a',1,'b',2)` → `{"a": 1, "b": 2}` — but the type name is not: `typeof(map('a',1))` → ``map(field { name: "entries", data_type: struct([…]) }, false)`` instead of Spark's `MAP<STRING, INT>`. | KAN-99 |
...
| Subscript `[]` on arrays and maps | Partial | Map subscript works (`SELECT map('a',1)['a']` → `1`). Array subscript is **1-based** here: `SELECT array(10,20,30)[0] AS z, array(10,20,30)[1] AS o` → `z=null, o=10`. Spark's `[]` is 0-based (expected `z=10, o=20`); `element_at` is the 1-based form. | KAN-118 |
...
The original internal planning document's §Stories assigned the placeholder IDs
`OXIDANT-DBR-001..020`. They mapped in order onto both the Jira keys used above and the story rows
of the epic's Jira import CSV (no longer kept in this repo), positional by row order.

| Plan ID | Jira | Summary |
|---------|------|---------|
| OXIDANT-DBR-001 | KAN-89 | Build Spark SQL coverage matrix (this document) |
| OXIDANT-DBR-002 | KAN-90 | Add platform-specific SQL corpus to parity harness |
...
| OXIDANT-DBR-019 | KAN-107 | Spark SQL parity ratchet CI job |
...
| KAN-118 | Bug | Array `[]` is 1-based; Spark is 0-based | array/map subscript |
...
KAN-94 / KAN-100 / KAN-105 acceptance criteria in the original plan and Jira import artifacts (no longer kept in this repo) were broadened so the matrix rows citing them stay inside those tickets' stated scope.
...
- **`PUT INTO`.** Not probed; marked N/A from the manual's description as a Unity Catalog volume
  operation.
- **Value-level equivalence against a reference implementation.** A probe "passing" means the
  statement executed and returned a plausible Spark-shaped result. Except where a row says
  otherwise, results were not diffed against a live reference warehouse. The
  `parity/baseline.json` ratchet is what does value-level comparison, against Spark v4.0.0 —
  value-level comparison against a live reference warehouse is exactly the hole KAN-90 and
  KAN-107 exist to fill.
