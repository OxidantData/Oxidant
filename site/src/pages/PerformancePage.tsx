...
const oxidant = benchmarks.engines.find((e) => e.key === "oxidant");
...
function ProvenanceTable({ engines }: { engines: Engine[] }) {
  return (
    <div className="overflow-x-auto rounded-oxidant border border-hairline">
  ...
  return (
    <section className="mt-16">
      <div className="max-w-3xl">
        <span className="oxidant-eyebrow">{eyebrow}</span>
      ...
      {!anyMeasured && (
        <div className="mt-6 rounded-oxidant border border-dashed border-hairline bg-bg-subtle p-4 text-sm text-muted">
...
export default function PerformancePage() {
  ...
  return (
    <div className="oxidant-container py-14">
      <div className="max-w-3xl">
        <span className="oxidant-eyebrow">Benchmarks</span>
        <h1 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">Performance</h1>
        <p className="mt-4 text-lg text-muted">
          ClickBench on a dedicated box, plus TPC-H / TPC-DS SF10 head-to-head: Oxidant distributed vs
      ...
      <section className="mt-12">
        <div className="max-w-3xl">
          <span className="oxidant-eyebrow">ClickBench</span>
          <h2 className="mt-2 text-2xl font-bold tracking-tight">Four-engine head-to-head</h2>
          <p className="mt-3 text-muted">
            Oxidant against Sail, Apache Spark, and Spark with the Gluten/Velox native backend — same
        ...
        {!anyMeasured && (
          <div className="mt-8 rounded-oxidant border border-dashed border-hairline bg-bg-subtle p-4 text-sm text-muted">
        ...
        <section className="mt-10">
          ...
          <p className="mb-4 max-w-2xl text-sm text-muted">
            Against Sail — the lean Rust peer and our honest north-star — Oxidant is the faster engine;
            Spark and Spark+Gluten are the broader context. And Oxidant runs the{" "}
            <strong className="text-body">full 43/43</strong>
            {oxidant?.totalAll ? ` (${oxidant.totalAll.toFixed(1)}s, zero failures)` : ""} — it wins{" "}
      ...
      <SuiteSection
        ...
        blurb="Official Q1–Q22 over the same SF10 Parquet bytes on S3, on the same 3-node spec (1x c6g.2xlarge + 2x m8g.2xlarge), re-run fresh the same day with identical temperature (2 runs/query, hot = run 2): Oxidant distributed in strict mode via Spark Connect — with the S3 disk cache (a Databricks/Snowflake-style local-NVMe object cache on every worker) — vs stock Apache Spark 3.5.6 on EMR 7.13.0/YARN, which re-reads S3 on every query. It is a sweep: Oxidant 15.3s vs Spark 87.3s hot — 82% faster, winning all 22 queries. The cache is not the win: strip it from both engines and Oxidant still takes 69.0s vs 87.3s (21% faster, 16 of 22) — the wins are the engine: real table statistics, per-query hash-join selection (auto, not per-deployment), driver-side predicate pushdown before the stage split, runtime-measured shuffle cardinalities, and a plan cache that plans each stage once per worker — leak-free. The one pass Spark still takes is the truly cold, cacheless stream (105.7s vs our 118.3s — its S3A prefetcher reads ahead, ours does not yet); with the cache on, even the first-touch pass is a dead heat (104.1s vs 105.7s). Correctness is table stakes and Oxidant meets it: 22/22 golden-clean against DuckDB SF10."
      ...
      <section className="mt-16">
        <div className="max-w-3xl">
          <span className="oxidant-eyebrow">Cache off — the control</span>
          <h2 className="mt-2 text-2xl font-bold tracking-tight">
            Same fight, no cache on either side
          </h2>
          <p className="mt-3 text-muted">
            The sections above run Oxidant with its S3 disk cache on (the default, and the
            ...
            engine: planning, join strategy, execution. Oxidant still wins both suites, and
            the only pass that flips is the truly cold stream, where Spark&apos;s S3A
            prefetcher reads ahead and Oxidant&apos;s first touch does not yet.
          </p>
        </div>
      </section>
      ...
      <SuiteSection
        ...
        blurb="The control for the headline: cache off on both engines, identical everything else. Oxidant 69.0s vs Spark 87.3s hot — 21% faster, winning 16 of 22. Compare the cache-on section: 15.3s vs 87.3s (82% faster) — the cache multiplies the win, it does not create it. Cold-vs-cold (first touches, nothing cached anywhere): Spark 105.7s vs Oxidant 118.3s — 11% to Spark, the S3A prefetch edge on the one streaming pass; every subsequent pass belongs to Oxidant."
        ...
        <div className="min-w-0">
          ...
          <ul className="space-y-2.5 text-sm text-muted">
            <li>
              ...
              <code className="text-body">s3://oxidant-artifacts-…/&#123;tpch,tpcds&#125;-sf10/</code>
            </li>
            <li>
            ...
            <li>
              <strong className="text-body">Oxidant compute.</strong> Oxidant Spark Connect driver + 2
              workers on EC2 (AL2023 arm64, the <code className="text-body">oxidant-sf10</code> ASGs),
            ...
            <li>
              ...
              spec as Oxidant. Tables registered as temp views over the same S3 prefix (the Glue DBs
              are schema-less oxidant registrations, so Spark infers from the parquet footers — same
