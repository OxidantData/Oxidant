  ...
  return (
    <div className="weft-container py-14">
      ...
      <SuiteSection
        ...
        blurb="Same hardware, same bytes, all 99 queries on both engines, same temperature. The SF10 marathon is a rout: Weft 65.6s vs Spark 282.8s hot (0.23x), winning 98 of 99 — from 1.31x down three days ago to 4.3x ahead. The latest jump came from driver-side predicate pushdown before the distributed stage split (Q78 7.6→1.4s, Q39 1.8→0.5s) on top of the S3 disk cache: each table lands on local NVMe once per worker instead of re-downloading ~250MB/node from S3 on every query — and cold-vs-cold Weft still wins (251.7s vs 308.1s first touches, because each table downloads once per cluster, not once per query). The one remaining Spark win (Q4 11.4 vs 8.8) is a UNION-shaped plan the splitter deliberately leaves unoptimized — the last compute-bound workstream. 99/99 end-to-end under strict mode, every query golden-validated against DuckDB SF10. Per-query chart omitted (99 bars); totals tell the story."
