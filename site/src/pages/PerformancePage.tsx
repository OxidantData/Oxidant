  ...
  return (
    <div className="weft-container py-14">
      ...
      <SuiteSection
        ...
        blurb="Official Q1–Q22 over the same SF10 Parquet bytes on S3, on the same 3-node spec (1x c6g.2xlarge + 2x m8g.2xlarge), re-run fresh the same day with identical temperature (2 runs/query, hot = run 2): Weft distributed in strict mode via Spark Connect — now with the S3 disk cache (a Databricks/Snowflake-style local-NVMe object cache on every worker) — vs stock Apache Spark 3.5.6 on EMR 7.13.0/YARN, which re-reads S3 on every query. It is a sweep: Weft 17.6s vs Spark 87.3s hot (0.20x), winning all 22 queries. Even cold-vs-cold (first touches, cache filling): 101.8s vs 97.2s (1.05x, the one lineitem-dominated pass). The rest is engineering: real table statistics, per-query hash-join selection, runtime-measured shuffle cardinalities, and a plan cache that plans each stage once per worker — leak-free. Correctness is table stakes and Weft meets it: 22/22 golden-clean against DuckDB SF10."
      ...
      <SuiteSection
        ...
        blurb="Same hardware, same bytes, all 99 queries on both engines, same temperature. The SF10 marathon is no longer close: Weft 75.1s vs Spark 282.8s hot (0.27x), winning 95 of 99 — from 1.31x down two days ago to 3.8x ahead. The last jump came from the S3 disk cache: each table lands on local NVMe once per worker instead of re-downloading ~250MB/node from S3 on every query — and cold-vs-cold Weft still wins (251.7s vs 308.1s first touches, because each table downloads once per cluster, not once per query). The four remaining Spark wins (Q78 7.6 vs 4.7, Q4 9.9 vs 8.2, Q72 5.2 vs 4.4, Q39 1.8 vs 1.3) are compute-bound join/aggregate work, not I/O — the next workstream. 99/99 end-to-end under strict mode, every query golden-validated against DuckDB SF10. Per-query chart omitted (99 bars); totals tell the story."
